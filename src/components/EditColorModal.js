...
import { updateColorApi } from "../redux/actions/color.actions";
const EditColorModal = ({ show, closeModal, color, name,id }) => {
  const [selectedColor, setColor] = useState(color);
  const [nameColor, setNameColor] = useState(name);
              ...
              <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button
                  onClick={() => {
                    ...
                    let data = {
                      nameColor: nameColor,
                      refColor: selectedColor,
                    };
                    dispatch(updateColorApi(id,data));
                    closeModal();
                  }}
                  type="button"
